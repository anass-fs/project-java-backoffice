document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupDemoButtons() {
        ...
        const demoAccounts = {
            admin: { email: 'admin@app.com', password: 'admin123', role: 'Admin' },
            user: { email: 'user@app.com', password: 'user123', role: 'User' }
        };

        if (demoAdminBtn) {
            demoAdminBtn.addEventListener('click', function() {
                loginWithDemo(demoAccounts.admin);
            });
        }

        if (demoUserBtn) {
            demoUserBtn.addEventListener('click', function() {
                loginWithDemo(demoAccounts.user);
            });
        }
    }

    
    function loginWithDemo(account) {
        const emailInput = document.getElementById('email');
        const passwordInput = document.getElementById('password');
        const roleInput = document.getElementById('role');
        ...
        if (emailInput && passwordInput && roleInput) {
            ...
            roleButtons.forEach(btn => {
                ...
                if (btn.getAttribute('data-role') === account.role) {
